...
void Client::_load()
{
    ...
    stmt.result_string(1024); // address
    stmt.result_string(255);  // email
    stmt.result_string(64);   // phone
    stmt.result_string(32);   // ico
    stmt.result_string(32);   // dic
    stmt.result_string(64);   // iban

    if (!stmt.fetch())
        throw std::runtime_error("Client not found");

    _name = stmt.get_string(0);
    _address = stmt.get_string(1);
    _email = stmt.get_string(2);
    _phone = stmt.get_string(3);
    _ico = stmt.get_string(4);
    _dic = stmt.get_string(5);
    _iban = IBAN(stmt.get_string(6));
}
